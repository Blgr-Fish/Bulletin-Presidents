...
void TableDecharge::choisirBulletins()
{
    ...
    for (auto &kv : p_tableHachageCandidatsDecharge)
    {
        ...
        if (std::max(bulletin.candidatSpol, (size_t)p_electeurOccupant->getSensiPolitique()) - std::min(bulletin.candidatSpol, (size_t)p_electeurOccupant->getSensiPolitique()) < distMax && bulletin.nombreDeBulletins > 0)
        {
